...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String apiUrl = dotenv.env['API_URL'] ?? 'http://192.168.0.145:5000';
  String geminiAPIKEY = dotenv.env['GEMINI_API_KEY'] ?? 'AIzaSyCAb-ps4sK-Gr7OJFX165_brS7Qdnq4-Pk';
...
Future<void> _shareDataToAPI(String base64Image) async {
  try {
    Dio dio = Dio();
    dio.options.connectTimeout = Duration(seconds: 200);
    dio.options.receiveTimeout = Duration(minutes: 20);

    var body = {'url': base64Image};

    // Use the selected type in the API endpoint
    String endpoint = '$apiUrl/predict_$_selectedType'; // Correctly reference the apiUrl variable
